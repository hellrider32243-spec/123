...
PUBLIC_HOST = _env("GRPC_PUBLIC_HOST", _env("PUBLIC_VLESS_HOST", "139.28.240.127"))
...
AMS_HOST = _env("AMS_HOST", "139.28.240.127")
...
_DEFAULT_HY2_HOST = f"wingsvpn.{AMS_HOST}.sslip.io" if AMS_HOST else "wingsvpn.139.28.240.127.sslip.io"
...
AMS_XHTTP_PORT = int(_env("AMS_XHTTP_PORT", "8443"))
AMS_XHTTP_SNI = _env("AMS_XHTTP_SNI", AMS_SNI)
...
AUTO_ALL_TAG = _env("AUTO_ALL_BALANCER_TAG", "AUTO_ALL")
AUTO_ALL_OBS_INTERVAL = _env("AUTO_ALL_OBS_INTERVAL", "45s")
AUTO_ALL_OBS_SAMPLING = int(_env("AUTO_ALL_OBS_SAMPLING", "3") or "3")
AUTO_ALL_TAG_PREFIX = "auto_"
...
def _auto_all_burst_observatory() -> dict[str, Any]:
    return {
        "subjectSelector": [AUTO_ALL_TAG_PREFIX],
        "pingConfig": {
            "connectivity": "http://connectivitycheck.gstatic.com/generate_204",
            "destination": "http://www.gstatic.com/generate_204",
            "interval": AUTO_ALL_OBS_INTERVAL,
            "sampling": AUTO_ALL_OBS_SAMPLING,
            "timeout": "5s",
        },
    }


def _auto_all_routing_rules() -> dict[str, Any]:
    """Тот же split-tunnel, что у остальных профилей, но всё «в VPN» идёт через
    балансер по четырём транспортам (TCP / XHTTP / gRPC / Hysteria2)."""
    base = _ultima_routing_rules()
    rules: list[dict[str, Any]] = []
    for rule in base["rules"]:
        r = dict(rule)
        if r.get("outboundTag") in ("proxy", "tg"):
            r.pop("outboundTag", None)
            r["balancerTag"] = AUTO_ALL_TAG
        rules.append(r)
    rules.append({"type": "field", "network": "tcp,udp", "balancerTag": AUTO_ALL_TAG})
    return {
        "domainMatcher": base.get("domainMatcher", "hybrid"),
        "domainStrategy": base.get("domainStrategy", "IPIfNonMatch"),
        "balancers": [
            {
                "tag": AUTO_ALL_TAG,
                "selector": [AUTO_ALL_TAG_PREFIX],
                "strategy": {
                    "type": "leastLoad",
                    "settings": {
                        "baselines": ["250ms", "500ms", "1s"],
                        "expected": 1,
                        "maxRTT": "2500ms",
                        "tolerance": 0.05,
...
def _base_config(
    ...
    *,
    ...
    known_tags = {str(o.get("tag") or "") for o in outbounds}
    if "tg" not in known_tags:
        # Hysteria2 и другие не-VLESS профили: отдельного TCP-канала для Telegram нет,
        # иначе правила на несуществующий outbound «tg» роняют все соединения Telegram.
        proxy_tag = str(proxy_outbound.get("tag") or "proxy")
        routing = _retag_rules(routing, {"tg": proxy_tag})
    return {
...
def _retag_rules(routing: dict[str, Any], mapping: dict[str, str]) -> dict[str, Any]:
    out = dict(routing)
    out["rules"] = [
        {**r, "outboundTag": mapping.get(str(r.get("outboundTag")), r.get("outboundTag"))}
        if "outboundTag" in r
        else r
...
def _xhttp_reality_outbound(
    ...
    *,
    ...
    """VLESS + XHTTP + Reality. Без fragment и без REST-обфускации — это не CDN/TLS."""
    return {
        "protocol": "vless",
        "settings": {
            "vnext": [
...
def build_nl_xhttp_config(
    ...
    *,
    ...
    display_name: Optional[str] = None,
    user_id: Optional[int] = None,
    server_description: str = "VLESS | XHTTP | Reality | :8443",
    **_: Any,
) -> dict[str, Any]:
    """VLESS + XHTTP stream-up + Reality — замена gRPC, обычно быстрее на мультиплексе."""
    remark = display_name or PROFILE_AMS_XHTTP
    outbound = _xhttp_reality_outbound(
        client_uuid,
        host=host or AMS_XHTTP_HOST or AMS_HOST,
        port=int(port or AMS_XHTTP_PORT),
        sni=sni or AMS_XHTTP_SNI or AMS_SNI,
        pbk=pbk or AMS_PBK,
        sid=sid or AMS_SID,
        fingerprint=fingerprint or AMS_XHTTP_FP or "chrome",
        path=path or AMS_XHTTP_PATH or "/",
        mode=mode or AMS_XHTTP_MODE or "stream-up",
    )
    return _base_config(
        remark,
        outbound,
...
def build_nl_auto_config(
    client_uuid: str,
    base_remark: str = COUNTRY_LABEL,
    *,
    display_name: Optional[str] = None,
    user_id: Optional[int] = None,
    server_description: str = "Сам выбирает самый быстрый канал",
    **_: Any,
) -> dict[str, Any]:
    """Один профиль на все четыре транспорта NL. burstObservatory меряет RTT
    ...
    for tag, cfg in sources:
        ...
        ob["tag"] = tag
        outbounds.append(ob)
    cfg = _balancer_config(
        remark,
        outbounds,
        meta=_happ_meta(user_id=user_id, extra={"serverDescription": server_description[:30]}),
        routing=_auto_all_routing_rules(),
        dns={"queryStrategy": "UseIPv4", "servers": ["8.8.8.8", "1.1.1.1"]},
        burst_observatory=_auto_all_burst_observatory(),
    )
...
def build_happ_json_subscription(
    ...
    *,
    ...
    auto_name = _env("VPN_PROFILE_NL_AUTO", "🇳🇱 🤖 Авто")
    profiles: list[dict[str, Any]] = [
        build_nl_auto_config(
            uuid,
            country,
            user_id=user_id,
